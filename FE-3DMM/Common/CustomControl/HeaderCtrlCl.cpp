...
void CHeaderCtrlCl::OnPaint()
{
	...
	{ 
		...
		int R = m_Rbegin,G = m_Gbegin,B = m_Bbegin;
		int R2 = m_Rend,G2 = m_Gend,B2 = m_Bend;

		CRect nRect(tRect);//拷贝尺寸到新的容器中 
		nRect.left++;//留出分割线的地方 
		//绘制立体背景 
		int interR,interG,interB;
		interR = ( R - R2 )/tRect.Height();
		interG = ( G - G2 )/tRect.Height();
		interB = ( B - B2 )/tRect.Height();
		...
		nFont.CreateFont( 13, 0, 0, 0, FW_BOLD, 0, 0, 0, ANSI_CHARSET, OUT_TT_PRECIS, 
	...
	//绘制立体背景 
	int interR,interG,interB;
	interR = ( R - R2 )/rtRect.Height();
	interG = ( G - G2 )/rtRect.Height();
	interB = ( B - B2 )/rtRect.Height();
