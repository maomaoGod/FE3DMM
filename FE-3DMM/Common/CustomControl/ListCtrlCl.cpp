...
, m_fontWith(0)
{
	m_cursor = AfxGetApp()->LoadCursor(134);
	m_color = RGB(0,0,0);
}
...
//   当当前ITEM   改变时发送消息，通知改变鼠标的光标   
BOOL   CListCtrlCl::OnSetCursor(CWnd*   pWnd,   UINT   nHitTest,   UINT   message)     
{   
...
void CListCtrlCl::InsertItemAndSetColor (int nItem, LPCTSTR lpszItem,int num)
{
	...
	if (nItem%2 == 1)
	{
		for (int i=0;i<num;i++)
		{
			SetItemColor( i ,nItem, RGB(148, 182, 225));  //设置单元颜色
		}
	}
}